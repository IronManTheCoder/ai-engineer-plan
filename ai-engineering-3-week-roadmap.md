...
A detailed review of the first draft flagged five real problems. Here's what changed:

| Problem flagged | Fix applied |
|---|---|
| Week 2 taught 5 frameworks in 7 days — shallow, not deep | Cut to two frameworks in depth (Vercel AI SDK, Mastra), MCP as required interop, Claude Agent SDK as a specialized tool for the coding agent, LangGraph.js moved to an optional appendix |
| Rebuilding disconnected demos in each framework wasted time | Days 8–12 now evolve **one running example** ("Support Assistant") across AI SDK → MCP → Mastra → Mastra workflows, with an explicit framework-comparison deliverable, not just repeated code |
| Evals only showed up on Day 14 | A lightweight eval set starts on **Day 6**, alongside the first RAG build, and gets extended (never rebuilt from scratch) through Day 20 |
| "Production-grade" was undefined | Added an explicit **minimum production bar** (below) that every capstone is graded against, and renamed the outcome "production-oriented portfolio applications" rather than claiming production-grade |
| Coding agent's security constraints were an afterthought | Security constraints are now part of the Day 13 design brief, not a stretch goal bolted on later |
| Day 21 tried to do six days of work in one | README, MCP-ification, and deployment are now done incrementally inside each capstone's days; Day 21 is just integration testing, final eval run, demo, and retro |
